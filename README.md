...
**清晰任务进入实施：跳过不必要的前置文档，但不能跳过授权和验证**

```text
使用 $product-development-workflow 处理以下清晰修改：将管理后台用户列表默认日期范围改为最近七个自然日，重置筛选时恢复同一范围，并补充相应测试。先只读反馈范围、非目标和验证方式，再请求实施授权；收到我的后续肯定回复后，只修改前端筛选默认值和测试，不创建分支、不提交、不调用外部系统。
```
...
明确只要求分析、诊断、审查、解释、状态检查或报告的请求，走七阶段之外的只读出口，可结束为“分析完成”“无需变更”“阻塞”或“建议进入后续阶段”；该路径不创建正式产物或修改状态。
...
阶段文档只在需要时读取，避免把探索、决策、计划、实施结果和完成证据混成同一份结论。目标、边界和验收明确且无需新增契约或独立计划的修改，可以在取得工作区修改与本地验证授权后直接进入阶段六，但必须经过阶段七才能声称完成。
